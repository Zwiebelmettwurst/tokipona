...
(function (DATA, MUSI, OPEN, LIPU, TP) {
  ...
  const T = {
    de: {
      ...
      weakHint: 'Nur das, was zuletzt gewackelt hat — Wörter und Konzepte, '
        + 'unabhängig davon, wann sie planmäßig dran wären.',
    ...
    en: {
      ...
      weakHint: 'Only what wobbled last time — words and concepts, no matter '
        + 'when they would be due.',
  ...
  function buildReviewSession() {
    const tasks = shuffle(dueKeys()).map(taskFromKey).filter(Boolean).slice(0, sizeOf().cap);
    return {
  ...
  function sizeCard() {
    const card = el(`
      <div class="card">
        <h2>${escape(t('sizeTitle'))}</h2>
        <p class="hint">${escape(t('sizeHint'))}</p>
        <div class="row"></div>
      </div>`);
    const row = card.querySelector('.row');
    const labels = { kurz: t('sizeShort'), mittel: t('sizeMedium'), lang: t('sizeLong') };
    SIZES.forEach((entry) => {
      const button = el(`<button class="ghost" data-picked="${entry.id === sizeOf().id}"
        >${escape(labels[entry.id])}</button>`);
      button.onclick = () => { state.size = entry.id; save(); render(); };
      row.append(button);
    });
    return card;
  }
